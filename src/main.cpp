...
void oscillator_gui(Oscillator* osc, const char* window_name, size_t idx) {
    ...
    if (ImGui::Begin(window_name)) {
        ...
        const ImVec2 child_size = {ImGui::GetContentRegionAvail().x * child_wcoeff, 0};
        {
            ...
            if (idx == 0) {
                parameter_knob(&osc->control_arr[0], params[2][idx], "Control 1###control1");
                ImGui::SameLine();
                parameter_knob(&osc->control_arr[1], params[3][idx], "Control 2###control2");
                ImGui::SameLine();
                parameter_knob(&osc->pcm_dwgs_wave, params[4][idx], "PCM/DWGS###pcm_dwgs_wave");
            } else {
                parameter_knob(&osc->semitone, params[2][idx], "Semitone###semitone", -24, 24);
                ImGui::SameLine();
                parameter_knob(&osc->tune, params[3][idx], "Tune###tune", -63, 63);
            }
            ImGui::EndChild();
        }
...

        {
            ...
            parameter_enum(&osc->wave, params[0][idx], "Wave###wave", &wave_enum[idx]);
            parameter_enum(&osc->osc_mod, params[1][idx], "Modulation###mod", &mod_enum[idx]);
            ImGui::EndChild();
        }
    }
